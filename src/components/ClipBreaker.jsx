...
export default function ClipBreaker() {
  ...
  const styles = {
    controlsBar: { display: "flex", flexWrap: "wrap", gap: 16, alignItems: "flex-end", padding: "20px 0", borderTop: "1px solid #222222", borderBottom: "1px solid #222222", marginBottom: 20 },
    ...
    slider: { WebkitAppearance: "none", width: 120, height: 4, borderRadius: 2, background: "#222222", cursor: "pointer" },
    processBtn: { background: "#4b0082", color: "#ffffff", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 12, fontWeight: 700, fontFamily: "inherit", letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer", marginLeft: "auto" },
    viewToggle: { display: "flex", gap: 4, background: "#1a1a1a", borderRadius: 8, padding: 4 },
    progressBar: { width: "100%", height: 3, background: "#1a1a1a", borderRadius: 2, overflow: "hidden", marginBottom: 20 },
    ...
    frameCard: (isSelected) => ({ position: "relative", borderRadius: 8, overflow: "hidden", cursor: "pointer", border: isSelected ? "2px solid #9370db" : "2px solid transparent", flexShrink: 0, background: "#1a1a1a" }),
    lightbox: { position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.95)", backdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 40 },
  };
    ...
    <div>
      {!videoSrc ? (
        <div style={styles.dropzone} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); handleDrop(e); }} onClick={() => fileInputRef.current?.click()}>
          ...
          <p style={{ fontSize: 14, color: "#888888" }}>Drop a video clip or <span style={{ color: "#9370db", textDecoration: "underline" }}>browse</span></p>
        ...
        <>
          ...
          <div style={styles.controlsBar}>
            ...
            <div style={styles.controlGroup}>
              ...
              <div style={styles.viewToggle}>
                ...
                <button style={{ background: view === VIEWS.VERTICAL ? "rgba(147,112,219,0.15)" : "transparent", color: view === VIEWS.VERTICAL ? "#9370db" : "#555555", border: "none", borderRadius: 6, cursor: "pointer", padding: "6px" }} onClick={() => setView(VIEWS.VERTICAL)}><IconVertical /></button>
          ...
          {processing && (<div style={styles.progressBar}><div style={{ width: `${progress}%`, height: "100%", background: "#4b0082" }} /></div>)}
          {frames.length > 0 && (
          ...
          {frames.length === 0 && !processing && (<div style={{ textAlign: "center", padding: "60px 20px", color: "#555555" }}><p>Configure settings and hit Break Clip</p></div>)}
        </>
      )}
      {selectedFrame !== null && frames[selectedFrame] && (
        <div style={styles.lightbox} onClick={() => setSelectedFrame(null)}>
          <img src={frames[selectedFrame].src} alt="Expanded frame" style={{ maxWidth: "90%", maxHeight: "85vh", borderRadius: 8, border: "1px solid #222222" }} onClick={(e) => e.stopPropagation()} />
          <span style={{ position: "absolute", bottom: 32, left: "50%", transform: "translateX(-50%)", background: "rgba(10,10,15,0.9)", color: "#9370db", fontSize: 13, fontWeight: 600, padding: "6px 16px", borderRadius: 6 }}>{formatTime(frames[selectedFrame].time)}</span>
        </div>
      )}
    </div>
  );
