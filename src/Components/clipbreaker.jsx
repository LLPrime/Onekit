...
function computeDiff(a, b) {
  ...
  const sampledPixels = Math.floor(len / 64);
  return totalDiff / (sampledPixels * 3);
}
...
function extractFramesInterval(video, canvas, ctx, intervalSec, quality) {
  return new Promise((resolve) => {
    ...
    const handleSeeked = () => {
      ...
      frames.push({ time: currentTime, src: dataUrl });
...
export default function ClipBreaker() {
  ...
  const processVideo = useCallback(async () => {
    ...
    try {
      ...
      alert("Failed to process video. Please try a shorter clip.");
    } finally {
  ...
  const styles = {
    root: { minHeight: "100vh", background: "#0a0a0f", color: "#e8e6e1", fontFamily: "'JetBrains Mono', monospace", position: "relative", overflow: "hidden" },
    container: { maxWidth: 960, margin: "0 auto", padding: "32px 20px", position: "relative", zIndex: 1 },
    ...
    dropzone: { border: "2px dashed #2a2a3a", borderRadius: 12, padding: "48px 24px", textAlign: "center", cursor: "pointer", transition: "all 0.3s ease", background: "rgba(255,255,255,0.01)" },
    ...
    select: { background: "#12121e", border: "1px solid #2a2a3a", borderRadius: 6, color: "#e8e6e1", padding: "8px 12px", fontSize: 12, fontFamily: "inherit", cursor: "pointer", outline: "none" },
    slider: { WebkitAppearance: "none", width: 120, height: 4, borderRadius: 2, background: "#2a2a3a", outline: "none", cursor: "pointer" },
    processBtn: { background: "linear-gradient(135deg, #5eead4, #2dd4bf)", color: "#0a0a0f", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 12, fontWeight: 700, fontFamily: "inherit", letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer", marginLeft: "auto", whiteSpace: "nowrap" },
    ...
    stripContainer: { display: "flex", gap: 12, overflowX: "auto", padding: "12px 0", scrollBehavior: "smooth" },
    gridContainer: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, padding: "12px 0" },
    frameCard: (isSelected) => ({ position: "relative", borderRadius: 8, overflow: "hidden", cursor: "pointer", border: isSelected ? "2px solid #5eead4" : "2px solid transparent", transition: "all 0.2s", flexShrink: 0, background: "#12121e" }),
  ...
  return (
    <div style={styles.root}>
      <div style={styles.container}>
        ...
        {!videoSrc ? (
          <div style={{ ...styles.dropzone, borderColor: dragging ? "#5eead4" : "#2a2a3a", background: dragging ? "rgba(94,234,212,0.03)" : "rgba(255,255,255,0.01)" }}
          ...
          >
            ...
            <p style={{ fontSize: 14, color: "#6a6a7a", marginTop: 12 }}>Drop a clip or <span style={{ color: "#5eead4", textDecoration: "underline" }}>browse</span></p>
          ...
          <>
            ...
            <div style={styles.controlsBar}>
              ...
              {method === "interval" ? (
                <div style={styles.controlGroup}>
                  <span style={styles.label}>Every {interval}s</span>
                  <input type="range" min={0.25} max={5} step={0.25} value={interval} onChange={(e) => setInterval_(parseFloat(e.target.value))} style={styles.slider} />
                </div>
              ) : (
                <div style={styles.controlGroup}>
                  <span style={styles.label}>Sensitivity {sensitivity}</span>
                  <input type="range" min={5} max={60} step={1} value={sensitivity} onChange={(e) => setSensitivity(parseInt(e.target.value))} style={styles.slider} />
                </div>
              )}
              <div style={styles.controlGroup}>
              ...
              <button style={{ ...styles.processBtn, opacity: processing ? 0.4 : 1, cursor: processing ? "not-allowed" : "pointer" }} onClick={processVideo} disabled={processing}>{processing ? "Breaking..." : "Break Clip"}</button>
            </div>
            {processing && (<div style={styles.progressBar}><div style={{ width: `${progress}%`, height: "100%", background: "linear-gradient(90deg, #5eead4, #2dd4bf)", transition: "width 0.3s ease" }} /></div>)}
            {frames.length > 0 && (
              <>
                <div style={view === VIEWS.STRIP ? styles.stripContainer : styles.gridContainer}>
                  {frames.map((f, i) => (
                    <div key={i} style={styles.frameCard(selectedFrame === i)} onClick={() => setSelectedFrame(i)}>
                      <img src={f.src} alt={`Frame at ${formatTime(f.time)}`} style={{ display: "block", width: view === VIEWS.STRIP ? 280 : "100%", height: "auto", objectFit: "cover" }} />
                      <span style={{ position: "absolute", bottom: 6, left: 6, background: "rgba(10,10,15,0.85)", color: "#5eead4", fontSize: 10, fontWeight: 600, padding: "3px 8px", borderRadius: 4 }}>{formatTime(f.time)}</span>
                      <span style={{ position: "absolute", top: 6, right: 6, background: "rgba(10,10,15,0.7)", color: "#6a6a7a", fontSize: 9, padding: "2px 6px", borderRadius: 3 }}>{i + 1}/{frames.length}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 24, padding: "12px 0", borderTop: "1px solid #1a1a2a", marginTop: 8, fontSize: 11, color: "#5a5a6a" }}>
                  <span>Frames: <span style={{ color: "#e8e6e1", fontWeight: 600 }}>{frames.length}</span></span>
                  <span>Method: <span style={{ color: "#e8e6e1", fontWeight: 600 }}>{method === "interval" ? `${interval}s interval` : `Scene detect (${sensitivity})`}</span></span>
                </div>
              </>
            )}
            {frames.length === 0 && !processing && (<div style={{ textAlign: "center", padding: "60px 20px", color: "#3a3a4a" }}><p style={{ fontSize: 13, marginBottom: 4 }}>Configure and hit Break Clip</p><p style={{ fontSize: 11 }}>Frames will appear here</p></div>)}
          </>
        )}
      </div>
      {selectedFrame !== null && frames[selectedFrame] && (
        <div style={styles.lightbox} onClick={() => setSelectedFrame(null)}>
          <button style={{ position: "absolute", top: 24, right: 24, color: "#6a6a7a", background: "rgba(255,255,255,0.05)", border: "1px solid #2a2a3a", borderRadius: 6, padding: "6px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }} onClick={(e) => { e.stopPropagation(); setSelectedFrame(null); }}><IconClose /> ESC</button>
          ...
          <span style={{ position: "absolute", bottom: 32, left: "50%", transform: "translateX(-50%)", background: "rgba(10,10,15,0.9)", color: "#5eead4", fontSize: 13, fontWeight: 600, padding: "6px 16px", borderRadius: 6 }}>{formatTime(frames[selectedFrame].time)} — Frame {selectedFrame + 1} of {frames.length}</span>
        </div>
      )}
      <style>{`input[type="range"]::-webkit-slider-thumb { -webkit-appearance: none; width: 14px; height: 14px; border-radius: 50%; background: #5eead4; cursor: pointer; border: 2px solid #0a0a0f; } ::-webkit-scrollbar { height: 6px; } ::-webkit-scrollbar-track { background: #12121e; } ::-webkit-scrollbar-thumb { background: #2a2a3a; border-radius: 3px; } @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;700&display=swap');`}</style>
    </div>
  );
}
